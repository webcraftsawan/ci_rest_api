...
class Interacts extends BaseController
{
	...
	public function index(){
		...
		$data = [];
		$data['title'] = 'Interacts';
		$data['description'] = '';
		$data['keywords'] = '';
		$data['form_action'] = base_url('admin/interacts/store');
		$data['user'] = $this->session->get('admin');
		$data['interacts'] = $this->common_model->get('interactions');

		if($this->session->get('admin')){
			return view('admin/interacts/index' , $data);
		}else{
			return redirect()->to(base_url('admin'));
		}
	}
	...
	public function delete($id){
		...
		$resp = $this->common_model->deleteData('interactions' , [ 'id' => $id ]);

		if($resp){
			
			$this->session->setFlashdata('success' , 'Interacts deleted');

			return redirect()->to(base_url('admin/interacts'));
	...
	public function view($id){
		$data = [];
		$data['title'] = 'View - Interacts';
		$data['description'] = '';
		$data['keywords'] = '';
		$data['form_action'] = base_url('admin/interacts/update/'.$id);
		$data['user'] = $this->session->get('admin');
		$data['resp'] = $this->common_model->get('interactions' , ['id' => $id]);
		return view('admin/interacts/view' , $data);
	}
