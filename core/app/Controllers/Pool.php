...
class Pool extends BaseController
{
	...
	public function index(){
		...
		$data['description'] = '';
		$data['keywords'] = '';
		$data['form_action'] = base_url('admin/pool/store');
		$data['user'] = $this->session->get('admin');
		$data['pools'] = $this->common_model->get('pools');
	...
	public function store(){
		...
		if(!$val){
			...
			$resp = $this->common_model->add('pools' , $postData);

			if($resp){
				
				$this->session->setFlashdata('success' , 'Pool created successfully');

				return redirect()->to(base_url('admin/pool/list'));
	...
	public function edit($id){
		...
		$data['description'] = '';
		$data['keywords'] = '';
		$data['form_action'] = base_url('admin/pool/update/'.$id);
		$data['user'] = $this->session->get('admin');

		$data['resp'] = $this->common_model->get('pools' , ['id' => $id]);
	...
	public function update($id){
		...
		if(!$val){
			...
			$resp = $this->common_model->edit('pools' , [ 'id' => $id ],  $postData);

			if($resp){
				
				$this->session->setFlashdata('success' , 'Pool updated successfully');
	...
	public function delete($id){
		...
		$resp = $this->common_model->deleteData('pools' , [ 'id' => $id ]);
